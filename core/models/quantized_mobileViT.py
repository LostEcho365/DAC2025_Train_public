...
class QAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # import pdb; pdb.set_trace()
        qkv = qkv.reshape(B, N, 3, self.num_heads, self.dim_head)  # [B, N, 3, num_heads, dim_head]
        qkv = qkv.permute(2, 0, 3, 1, 4)  # [3, B, num_heads, N, dim_head]
        q, k, v = qkv[0], qkv[1], qkv[2]  # Each: [B, num_heads, N, dim_head]
        # Scaled dot-product attention
        attn_scores = self.quantized_matmul(q, k.transpose(-2, -1)) * self.scale  # [B, num_heads, N, N]
        attn_probs = torch.softmax(attn_scores, dim=-1)  # [B, num_heads, N, N]

        # Apply attention weights to values
        attn_output = self.quantized_matmul(attn_probs, v)  # [B, num_heads, N, dim_head]

        # Concatenate heads and project back to dim
        attn_output = attn_output.transpose(1, 2).contiguous().reshape(B, N, self.inner_dim)  # [B, N, inner_dim]
        # import pdb; pdb.set_trace()
        output = self.proj(attn_output)  # [B, N, dim]
        # import pdb; pdb.set_trace()
        # output = self.proj(attn_output)
        # qkvout = self.qkv(x).chunk(3, dim=-1)
        # q, k, v = map(lambda t: rearrange(t, 'b p n (h d) -> b p h n d', h = self.num_heads), qkvout)
        # dots = self.quantized_matmul(q, k.transpose(-1, -2)) * self.scale
        # attn = torch.softmax(dots, dim=-1)
        # out = self.quantized_matmul(attn, v)
...
class QMobileViT(QBaseModel):
    def __init__(
        ...
        assert image_height % patch_size[0] == 0 and image_width % patch_size[1] == 0, "Image dimensions must be divisible by the patch size"
        ...
        # Stage 9
        self.conv_last = conv1x1(channels[-2], channels[-1], conv_cfg=conv_cfg, device=device, norm_cfg=norm_cfg, act_cfg=act_cfg)
